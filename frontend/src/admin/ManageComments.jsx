...
const ManageComments = () => {
    ...
    news.forEach(item => {
        if (item.comments) {
            item.comments.forEach(c => allComments.push({ ...c, sourceTitle: item.title, sourceType: 'news', sourceId: item._id }));
        }
    });

    videos.forEach(item => {
        if (item.comments) {
            item.comments.forEach(c => allComments.push({ ...c, sourceTitle: item.title, sourceType: 'video', sourceId: item._id }));
        }
    });

    movies.forEach(item => {
        if (item.comments) {
            item.comments.forEach(c => allComments.push({ ...c, sourceTitle: item.title, sourceType: 'movie', sourceId: item._id }));
        }
    });

    celebs.forEach(item => {
        if (item.comments) {
            item.comments.forEach(c => allComments.push({ ...c, sourceTitle: item.name, sourceType: 'celeb', sourceId: item._id }));
        }
    });
                                ...
                                <tr key={comment._id || `comment-${index}`} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center justify-center gap-2">
                                            <i className="fas fa-heart text-primary-red text-[10px]"></i>
                                            <input 
                                                type="number" 
